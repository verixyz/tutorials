...
exports.generateScenarioCommon = ({ environment, scenarioId, unlockingScriptId, }) => {
    ...
    const testedInputs = extendedScenario.transaction.inputs.filter((input) => input.unlockingBytecode === true);
    ...
    const testedInputIndex = extendedScenario.transaction.inputs.findIndex((input) => input.unlockingBytecode);
    ...
    (output, index) => {
        ...
        const result = compile_1.compileScriptRaw({ data, environment, scriptId });
        if (!result.success) {
            ...
            spendingTransaction: {
                inputs: extendedScenario.transaction.inputs.map((input) => {
                    ...
                    return ({
                        ...
                        unlockingBytecode: hex_1.hexToBin(typeof input.unlockingBytecode === 'string'
