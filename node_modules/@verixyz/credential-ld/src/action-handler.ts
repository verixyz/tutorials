...
import { schema, verixyzLdSignature } from './'
...
const debug = Debug('verixyz:w3c:action-handler')
...
export class CredentialIssuerLD implements IAgentPlugin {
  ...
  constructor(options: { contextMaps: RecordLike<OrPromise<ContextDoc>>[]; suites: verixyzLdSignature[] }) {
    this.ldCredentialModule = new LdCredentialModule({
      ldContextLoader: new LdContextLoader({ contextsPaths: options.contextMaps }),
      ldSuiteLoader: new LdSuiteLoader({ verixyzLdSignatures: options.suites }),
    })
