...
const debug = Debug('verixyz:w3c:ld-credential-module')
...
export class LdCredentialModule {
   ...
   * - Key Manager and Verification Methods: verixyz currently implements no link between those.
   */
  ...
  getDocumentLoader(context: IAgentContext<IResolver>, attemptToFetchContexts: boolean = false) {
    return extendContextLoader(async (url: string) => {
      ...
      if (url.toLowerCase().startsWith('did:')) {
        ...
        // currently verixyz LD suites can modify the resolution response for DIDs from
        // the document Loader. This allows to fix incompatibilities between DID Documents
        // and LD suites to be fixed specifically within the verixyz LD Suites definition
